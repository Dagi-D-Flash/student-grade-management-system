...
/*Collapsible sidebar with smooth animations.*Expanded=240 px:icon+label visible.*Collapsed=56 px:icon only,label hidden,tooltip shown.*/
...
public class CollapsibleSidebar extends JPanel {
    ...
    public void toggle() {
        ...
        animTimer.addActionListener(e -> {
            ...
            int w = startW + (int) ((endW - startW) * t);
            ...
            if (step[0] >= ANIM_STEPS) {
                ...
                brandTextPanel.setVisible(expanded);
    ...
    public void setSelectedIndex(int index) {
        if (index >= 0 && index < navButtons.size()) {
            // Find the corresponding row panel
            Component[] components = navPanel.getComponents();
            int rowIndex = 0;
            for (Component comp : components) {
                if (comp instanceof JPanel && rowIndex == index) {
                    JPanel row = (JPanel) comp;
                    Component[] rowComponents = row.getComponents();
                    JLabel iconLbl = null, textLbl = null;
                    for (Component rowComp : rowComponents) {
                        if (rowComp instanceof JLabel) {
                            JLabel lbl = (JLabel) rowComp;
                            if (lbl.getHorizontalAlignment() == SwingConstants.CENTER) {
                                iconLbl = lbl;
                            } else {
                                textLbl = lbl;
                            }
                    ...
                    if (iconLbl != null && textLbl != null) {
                        setActiveRow(row, iconLbl, textLbl, navButtons.get(index));
                    }
                    break;
                }
