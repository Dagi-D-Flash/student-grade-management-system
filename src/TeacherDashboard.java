...
public class TeacherDashboard extends JFrame {
    ...
    public TeacherDashboard(User user) {
        ...
        // Double-clicking a student in My Students → navigate to Enter Grades and select them
    ...
    private JPanel buildHomeCard() {
        ...
        // Clickable "No Grades Yet" card — navigates to My Students with ungraded filter
        ...
        pendingCard.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (studentsPanel != null) studentsPanel.showUngraded();
    ...
    private void refreshHomeCard() {
        new SwingWorker<int[], Void>() {
            protected int[] doInBackground() throws Exception {
                ...
                try (java.sql.Connection conn = DBConnection.getConnection()) {
                    try (java.sql.PreparedStatement ps = conn.prepareStatement(sqlCourses)) {
                        ...
                        try (java.sql.ResultSet rs = ps.executeQuery()) { if (rs.next()) courses = rs.getInt(1); }
                    }
                    try (java.sql.PreparedStatement ps = conn.prepareStatement(sqlStudents)) {
                        ps.setInt(1, currentUser.getId());
                        try (java.sql.ResultSet rs = ps.executeQuery()) { if (rs.next()) students = rs.getInt(1); }
                    }
                    try (java.sql.PreparedStatement ps = conn.prepareStatement(sqlPending)) {
                        ps.setInt(1, currentUser.getId());
                        try (java.sql.ResultSet rs = ps.executeQuery()) { if (rs.next()) pending = rs.getInt(1); }
                    }
                }
            ...
            protected void done() {
                try {
                    int[] v = get();
                    if (valCourses  != null) valCourses.setText(String.valueOf(v[0]));
                    if (valStudents != null) valStudents.setText(String.valueOf(v[1]));
                    if (valPending  != null) valPending.setText(String.valueOf(v[2]));
